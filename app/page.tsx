 ...
 <div className="mobile-link" onClick={() => nav("home")}>🏠 Home</div>
  ...
  <div className="page-wrap" style={{maxWidth:440}}>
    ...
    <div style={{background:'var(--card)',borderRadius:'var(--radius)',padding:28,border:'1.5px solid var(--border)',boxShadow:'var(--shadow)'}}>
      ...
      {authSuccess ? (
        <div style={{background:'#d1fae5',color:'#059669',padding:'14px 16px',borderRadius:8,fontSize:13,fontWeight:600,textAlign:'center',marginBottom:16}}>
          {authSuccess}
        </div>
      ) : (
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          <button
            ...
            style={{background:'var(--primary)',color:'#fff',border:'none',borderRadius:'var(--radius-sm)',padding:'13px',fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:'Inter,sans-serif',width:'100%',opacity:authLoading?0.7:1}}
          >
...
<button
  onClick={async()=>{
    const res = await fetch(`${SUPABASE_URL}/auth/v1/authorize?provider=google&redirect_to=${window.location.origin}`,{
      headers:{'apikey':SUPABASE_KEY}
    });
    window.location.href = `${SUPABASE_URL}/auth/v1/authorize?provider=google&redirect_to=${window.location.origin}`;
  }}
  style={{width:'100%',background:'#fff',color:'#374151',border:'1.5px solid var(--border)',borderRadius:'var(--radius-sm)',padding:'12px',fontSize:14,fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif',display:'flex',alignItems:'center',justifyContent:'center',gap:10,boxShadow:'var(--shadow)'}}
>
